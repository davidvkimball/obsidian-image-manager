...
export class RemoteSearchModal extends Modal {
	...
	onOpen(): void {
		...
		loaderIcon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-loader-circle"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>';
	...
	private renderPagination(): void {
		...
		if (hasMore || this.currentPage > 1) {
			...
			if (this.currentPage > 1) {
				const prevBtn = pagination.createEl('button', { cls: 'btn', text: '← PREV' });
	...
	private renderError(message: string): void {
		if (!this.imagesList) return;
		this.imagesList.empty();
		const errorDiv = this.imagesList.createDiv({ cls: 'no-result-container' });
